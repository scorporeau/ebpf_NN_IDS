...
static inline int parse_update(struct xdp_md *ctx, struct feature_vector *fv, struct netevent *nete)
{
    ...
    //3bis : fill net_event vector with features if it exists (if debug)
    if (nete) {
...
int xdp_trace_net_event(struct xdp_md *ctx)
{
    ...
    __u32 *fv ;//create a nb_features sized array of __u32
    int ret;

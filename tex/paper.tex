...
In order to measure execution time and throughput of each designed ebpf script, we had to develop a strict protocol to apply. As said, we are measuring two different things in order to evaluate each script and version of it.
...
In order to measure throughput, iperf3 showed to be an efficient and easy-to-use module in order to emulate max throughput \cite{bachl_flow-based_2022}.

Then, we also wanted to measure execution time of code parts. Each ebpf script used is divided in 2 parts : parsing and classification. We ran many different tests, the results are showed in the according section.

...
IT SEEMS TO AHVE SOME ISSUES. The main issue is that the GRO is a linux feature that allows to reconstruct packets up to 65535 Bytes, but the GRO happened after the XDP hook, thus giving packets of max size 1500 to the XDP script. This issue is significantly increasing the numver of packets passed through the XDP script, thus increasing the need to go through all the parsing and classification process for a given number of transmitted Bytes. That might be the reason why we see a significant decrease of throughput for the XDP script, but a signigicant input in the execution times.

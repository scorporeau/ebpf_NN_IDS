...
def DT_export_c_header(clf: DecisionTreeClassifier, max_depth: int, out_path: str, report=None):
    ...
    dt_node.feature byte layout:
        bit 7 (MSB)  : pass_left  — 1 = pass if feature <= threshold, 0 = drop
        bit 6        : pass_right
        bit 6        : is_defined - 1 = defined node, 0 = unused
        bits 5..0    : feature index
    ...
    with open(out_path+"dt_params.h", "w") as f:
        ...
        for i, (feat, thresh) in enumerate(nodes):
            ...
            comment = (
                f"leaf / undef"
                if is_leaf
                else f"feat_id={feat_idx} thresh={thresh} pass_left={pass_left} pass_right = {pass_right}"
            )

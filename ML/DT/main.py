...
def write_update_params_sh(nodes: list, out_path: str):
    ...
    with open(sh_path, "w") as f:
        ...
        #assert map_id is given by the user
        f.write('if [ -z "$1" ]; then\n')
        f.write('    echo "Usage: sudo $0 <map_id>"\n')
        f.write('    exit 1\n')
        f.write('fi\n\n')

        #writing the raw hex data as lists (for easier reading of script)
        f.write('MAP_ID="$1"\n')
        f.write(f'NODE_NB={len(nodes)}\n\n')
        f.write('KEYS_MAP0=(\n')
        for i in range(len(nodes)):
            key_hex = u32_to_hex_le(i + 1)
            f.write(f'    "{key_hex}"\n')
        f.write(')\n\n')
        f.write('KEYS_MAP1=(\n')
        for i in range(len(nodes)):
            key_hex = u32_to_hex_le(len(nodes) + 1 + i)
            f.write(f'    "{key_hex}"\n')
        f.write(')\n\n')
        f.write('read_current_map() {\n')
        f.write('    local out raw\n')
        f.write('    out=$(bpftool map lookup id "$MAP_ID" key hex 00 00 00 00 2>/dev/null) || return 1\n\n')
        f.write('    raw=$(printf "%s" "$out" | grep -oE \'"value"[[:space:]]*:[[:space:]]*[0-9]+\' | tail -n1 | sed -E \'s/.*:[[:space:]]*([0-9]+).*/\\1/\')\n')
        f.write('    if [ -z "$raw" ]; then\n')
        f.write('        raw=$(printf "%s" "$out" | grep -oE "([0-9A-Fa-f]{2} ){3}[0-9A-Fa-f]{2}" | tail -n1)\n')
        f.write('        if [ -z "$raw" ]; then return 2; fi\n')
        f.write('        if [ "$raw" = "00 00 00 00" ]; then\n')
        f.write('            printf "0"\n')
        f.write('        else\n')
        f.write('            printf "1"\n')
        f.write('        fi\n')
        ...
        for i, node_value in enumerate(nodes):
            ...
            f.write(f'    "{comment}"\n')
        f.write(')\n\n')
        f.write('raw_used=$(read_current_map)\n')
        f.write('if [ $? -ne 0 ]; then\n')
        f.write('    echo "Warning: failed to read current map selection from key 0; defaulting to map 0"\n')
        f.write('    raw_used=0\n')
        f.write('fi\n\n')
        f.write('USED_MAP=$raw_used\n')
        f.write('echo "current DT map in use : $USED_MAP"\n\n')
        f.write('if [ "$USED_MAP" -eq 1 ]; then\n')
        f.write('    KEYS=("${KEYS_MAP0[@]}")\n')
        f.write('    TARGET_MAP=0\n')
        f.write('else\n')
        f.write('    KEYS=("${KEYS_MAP1[@]}")\n')
        f.write('    TARGET_MAP=1\n')
        f.write('fi\n\n')
        f.write('echo "filling DT map $TARGET_MAP"\n\n')
        f.write('for i in $(seq 0 $((NODE_NB - 1))); do\n')
        f.write('    IFS=" " read -r k0 k1 k2 k3 <<< "${KEYS[i]}"\n')
        f.write('    IFS=" " read -r v0 v1 v2 v3 <<< "${VALUES[i]}"\n')
        f.write('    echo "# ${COMMENTS[i]}"\n')
        f.write('    bpftool map update id "$MAP_ID" key hex "$k0" "$k1" "$k2" "$k3" value hex "$v0" "$v1" "$v2" "$v3"\n')
        f.write('    echo\n')
        f.write('done\n\n')
        f.write('if [ "$TARGET_MAP" -eq 0 ]; then\n')
        f.write('    echo "# switch active DT map selector to map 0"\n')
        f.write('    bpftool map update id "$MAP_ID" key hex 00 00 00 00 value hex 00 00 00 00\n')
        f.write('else\n')
        f.write('    echo "# switch active DT map selector to map 1"\n')
        f.write('    bpftool map update id "$MAP_ID" key hex 00 00 00 00 value hex 01 00 00 00\n')
        f.write('fi\n\n')
